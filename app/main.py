...
from .db import (
    ...
    add_post_tags, get_post_tags, get_post_stats, get_people, get_profile_settings, update_profile,
...
def build_info():
    ...
    date_label = datetime.fromtimestamp(latest_mtime or __import__('time').time(), tz=timezone.utc).strftime('%d.%m.%y')
    short_hash = digest.hexdigest()[:6].upper()
    return {'label': f'build {date_label} · {short_hash}', 'hash': short_hash}
...
def get_post_endpoint(post_id: int, x_telegram_init_data: str | None = Header(default=None)):
    ...
    post_data.update(get_post_stats(post_id, current['id']))
    return post_data
