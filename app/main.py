...
from .db import (
    ...
    view_story, get_story, get_admin_overview, admin_search_users, set_user_verified,
    set_user_admin, set_user_suspended, delete_all_user_posts, sync_owner_flags,
)
...
def build_info():
    import time
    stamp = time.strftime('%d.%m.%y', time.gmtime())
    parts = []
    for path in (
        Path(__file__),
        Path(__file__).with_name('db.py'),
        ...
        try:
            ...
            pass
    digest = hashlib.sha1('|'.join(parts).encode('utf-8')).hexdigest()[:6].upper()
    return {'label': f'build {stamp} · {digest}', 'hash': digest}
...
def current_user(x_telegram_init_data: str | None):
    ...
    upsert_user(user)
    sync_owner_flags(user['id'])
    saved = get_user(user['id']) or user
...
def notify(recipient_id, actor_id, kind, text, post_id=None, comment_id=None):
    ...
    if notification_id:
        ...
        threading.Thread(
            target=safe_push,
            args=(recipient_id, text, post_id),
            daemon=True,
        ).start()
...
async def post(
    ...
    if not file.filename:
        raise HTTPException(status_code=400, detail='Для публикации нужно выбрать фото.')
    ...
    try:
        ...
        raise HTTPException(status_code=507, detail=f'Не удалось сохранить изображение: {exc}')
    ...
    raw_tag_ids = [
        uid for uid in set(raw_tag_ids)
        if uid != user['id'] and get_user(uid)
    ]

    try:
        post_id = create_post(user['id'], filename, caption)
    except Exception as exc:
        try:
            target.unlink(missing_ok=True)
        except Exception:
            pass
        raise HTTPException(status_code=500, detail=f'Не удалось сохранить публикацию: {exc}')

    if raw_tag_ids:
        try:
            add_post_tags(post_id, raw_tag_ids)
            name = str(user.get('first_name', 'Пользователь')).strip()
            for tagged_user_id in raw_tag_ids:
                notify(
                    tagged_user_id,
                    user['id'],
                    'tag',
                    f'{name} отметил(а) вас в публикации.',
                    post_id=post_id,
                )
        except Exception:
            pass
...
@app.get('/api/users/{user_id}/share-link')
def profile_share_link(
    user_id: int,
    x_telegram_init_data: str | None = Header(default=None),
):
    current_user(x_telegram_init_data)
    ...
    if not bot_username and BOT_TOKEN:
        try:
            ...
            with urllib.request.urlopen(req, timeout=1.5) as response:
                ...
                bot_username = ((data.get('result') or {}).get('username') or '').strip().lstrip('@')
        except Exception:
            bot_username = ''

    if bot_username:
        return {
            'url': f'https://t.me/{bot_username}?startapp=profile_{user_id}',
    }

    domain = public_app_url() or ''
    return {'url': f'{domain}/?profile={user_id}'}
...
def user_friends(user_id: int, x_telegram_init_data: str | None = Header(default=None)):
    current_user(x_telegram_init_data)
...
def require_admin(x_telegram_init_data: str | None):
    user = current_user(x_telegram_init_data)
    saved = get_user(user['id']) or user
    if not saved.get('is_admin'):
        raise HTTPException(status_code=403, detail='Admin access required')
    return saved


def require_owner(x_telegram_init_data: str | None):
    user = current_user(x_telegram_init_data)
    saved = get_user(user['id']) or user
    if not saved.get('is_owner'):
        raise HTTPException(status_code=403, detail='Owner access required')
    return saved


@app.get('/api/admin/me')
def admin_me(x_telegram_init_data: str | None = Header(default=None)):
    user=current_user(x_telegram_init_data); saved=get_user(user['id']) or user
    return {'is_admin': bool(saved.get('is_admin')), 'is_owner': bool(saved.get('is_owner')), 'verified': bool(saved.get('verified'))}


@app.get('/api/admin/overview')
def admin_overview(x_telegram_init_data: str | None = Header(default=None)):
    admin=require_admin(x_telegram_init_data)
    return {'me': {'id': admin['id'], 'is_admin': bool(admin.get('is_admin')), 'is_owner': bool(admin.get('is_owner'))}, 'stats': get_admin_overview()}


@app.get('/api/admin/users')
def admin_users(q: str = '', x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data)
    return admin_search_users(q)


@app.post('/api/admin/users/{user_id}/verified')
def admin_verified(user_id: int, enabled: int = Form(1), x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data); target=get_user(user_id)
    if not target: raise HTTPException(status_code=404, detail='User not found')
    set_user_verified(user_id, bool(enabled)); return {'ok': True, 'verified': bool(enabled)}


@app.post('/api/admin/users/{user_id}/suspend')
def admin_suspend(user_id: int, enabled: int = Form(1), x_telegram_init_data: str | None = Header(default=None)):
    admin=require_admin(x_telegram_init_data); target=get_user(user_id)
    if not target: raise HTTPException(status_code=404, detail='User not found')
    if target.get('is_owner') and not admin.get('is_owner'): raise HTTPException(status_code=403, detail='Cannot suspend owner')
    if int(user_id)==int(admin['id']): raise HTTPException(status_code=400, detail='Cannot suspend yourself')
    set_user_suspended(user_id, bool(enabled)); return {'ok': True, 'suspended': bool(enabled)}


@app.post('/api/admin/users/{user_id}/admin')
def admin_role(user_id: int, enabled: int = Form(1), x_telegram_init_data: str | None = Header(default=None)):
    require_owner(x_telegram_init_data); target=get_user(user_id)
    if not target: raise HTTPException(status_code=404, detail='User not found')
    if target.get('is_owner'): raise HTTPException(status_code=400, detail='Owner role cannot be changed')
    set_user_admin(user_id, bool(enabled)); return {'ok': True, 'is_admin': bool(enabled)}


@app.delete('/api/admin/users/{user_id}/posts')
def admin_delete_user_posts(user_id: int, x_telegram_init_data: str | None = Header(default=None)):
    admin=require_admin(x_telegram_init_data); target=get_user(user_id)
    if not target: raise HTTPException(status_code=404, detail='User not found')
    if target.get('is_owner') and not admin.get('is_owner'): raise HTTPException(status_code=403, detail='Cannot modify owner posts')
    paths=delete_all_user_posts(user_id)
    for path in paths:
        try:
            file=UPLOADS_DIR/Path(path).name
