...
function postImageMarkup(path, postId = null, ownerId = null) {
  ...
  return `<img class="post-image" src="${src}" data-image-src="${src}" data-post-id="${postId || ''}" data-owner-id="${ownerId || ''}" loading="lazy" alt="Фото">`;
}

const HEART_SVG = '<svg class="action-svg heart-svg" viewBox="0 0 28 28" aria-hidden="true"><path d="M14 24.1 4.1 14.4C-1 9.2 2.1 2.5 7.5 2.5c2.8 0 5.1 1.6 6.5 3.6 1.4-2 3.7-3.6 6.5-3.6 5.4 0 8.5 6.7 3.4 11.9L14 24.1Z"></path></svg>';
const COMMENT_SVG = '<svg class="action-svg comment-svg" viewBox="0 0 28 28" aria-hidden="true"><path d="M23.5 13.2c0 5.5-4.6 9.9-10.3 9.9-1.7 0-3.3-.4-4.8-1.1L3.5 24l1.4-4.3a9.5 9.5 0 0 1-2-6.2C2.9 7.9 7.5 3.5 13.2 3.5s10.3 4.2 10.3 9.7Z"></path></svg>';
function feedLikeMarkup(post) { return `<button class="like ${post.liked ? 'liked' : ''}" onclick="likePost(${post.id}, this)" aria-label="Нравится">${HEART_SVG}<span>${post.likes}</span></button>`; }
function feedCommentMarkup(post) { return `<button class="comment-toggle" type="button" onclick="toggleComments(${post.id})" aria-label="Комментарии">${COMMENT_SVG}<span>${post.comments || 0}</span></button>`; }
  ...
  for (const post of posts) {
    html.push(`
      <article class="post" id="post-${post.id}">
        ...
        <div class="meta">${feedLikeMarkup(post)}${feedCommentMarkup(post)}</div>
...
function setViewerLikeState(liked, count) {
  ...
  if (icon) icon.innerHTML = HEART_SVG;
...
(async () => {
  ...
  if (!image) return;
  const src = image.dataset.imageSrc || image.currentSrc || image.src;
  const postId = Number(image.dataset.postId || 0);
  const ownerId = Number(image.dataset.ownerId || 0);
  if (!src) return;
  event.preventDefault();
  event.stopPropagation();
