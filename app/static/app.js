...
async function loadBuildNumber() {
  ...
  setTimeout(async () => {
    if (el.textContent === 'build') {
      try {
        const retry = await api('/api/build');
        if (retry.ok) { const data = await retry.json(); el.textContent = data.label || 'build'; }
      } catch (_) {}
    }
  }, 1200);
}
...
document.getElementById('theme-menu-btn')?.addEventListener('pointerup', e => {
  ...
  try {
    ...
    box.innerHTML = posts.map(post => `
      <article class="post" id="post-${post.id}">
        ...
        ${postImageMarkup(post.image_path, post.id, post.user_id)}
        ${post.caption ? `<p class="caption">${esc(post.caption)}</p>` : ''}
        ${renderPostTags(post.tags)}
        <div class="meta">
          <button class="like ${post.liked ? 'liked' : ''}" onclick="likePost(${post.id}, this)" aria-label="Нравится">
            ${post.liked ? '♥' : '♡'} <span>${post.likes}</span>
          </button>
          <button class="comment-toggle" type="button" onclick="toggleComments(${post.id})" aria-label="Комментарии">💬 ${post.comments || 0}</button>
        </div>
    ...
    box.innerHTML = '<div class="empty">Не удалось загрузить ленту. Проверь соединение и попробуй ещё раз.</div>';
  }
}
...
async function openProfileFriends(userId) {
  ...
  if (!res.ok) {
    let message = 'Не удалось загрузить друзей.';
