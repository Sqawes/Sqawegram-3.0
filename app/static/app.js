...
function showView(viewId, loadData = true) {
  ...
  if (!loadData) return;
  if (viewId === 'feed-view') { loadFeed(); loadStories(); loadNews(); }
...
document.getElementById('refresh-btn')?.addEventListener('click', () => { loadFeed(); loadStories(); loadNews(); });
...
async function openProfile(userId) {
  ...
  const u = data.user;
  const settingsButton = me && Number(me.id) === Number(u.id) ? '<div class="profile-actions"><button class="ghost" onclick="showView(\'settings-view\')">Настройки</button></div>' : '';
  p.innerHTML = `<div class="profile-back"><button class="ghost" type="button" onclick="showView('feed-view')">← Назад</button></div><div class="profile-head"><img class="profile-avatar" src="${avatar(u.photo_url)}"><div><div class="profile-name">${esc(u.first_name)} ${esc(u.last_name || '')}</div><div class="profile-stats">${u.username ? '@' + esc(u.username) : 'Telegram user'}</div><div class="profile-stats">${data.friend_count} друзей · ${data.posts.length} постов</div>${onlineText(u.last_seen)}</div></div><div class="friend-button">${friendButtonForProfile(u.id, data.friend_status)}</div>${settingsButton}<div class="grid">${data.posts.length ? data.posts.map(post => postImageMarkup(post.image_path, post.id)).join('') : '<div class="empty">Пока нет публикаций.</div>'}</div>`;
...
async function openOwnProfileFromUser(me) {
  const data = await getUserProfile(me.id); if (!data) return;
  const u = data.user;
  document.getElementById('profile').innerHTML = `<div class="profile-head"><img class="profile-avatar" src="${avatar(u.photo_url)}"><div><div class="profile-name">${esc(u.first_name)} ${esc(u.last_name || '')}</div><div class="profile-stats">${u.username ? '@' + esc(u.username) : ''}</div><div class="profile-stats">${data.friend_count} друзей · ${data.posts.length} постов</div>${onlineText(u.last_seen)}</div></div><div class="profile-actions"><button class="primary" onclick="showView('settings-view')">Настройки профиля</button></div><div class="grid">${data.posts.length ? data.posts.map(p => postImageMarkup(p.image_path, p.id)).join('') : '<div class="empty">У тебя пока нет публикаций.</div>'}</div>`;
}
...
async function loadStories() {
  const box = document.getElementById('stories-strip'); if (!box) return;
  const res = await api('/api/stories'); if (!res.ok) { box.innerHTML = ''; return; }
  stories = await res.json();
  const groups = new Map(); stories.forEach(s => { if (!groups.has(s.user_id)) groups.set(s.user_id, []); groups.get(s.user_id).push(s); });
  box.innerHTML = `<button class="story-add" type="button" onclick="chooseStoryPhoto()"><span>＋</span><small>Ваша история</small></button>` + [...groups.values()].map(group => { const s = group[0]; return `<button class="story-bubble" type="button" onclick="openStoryForUser(${s.user_id})"><img src="${avatar(s.photo_url)}"><span>${esc(s.first_name)}</span></button>`; }).join('');
}
function chooseStoryPhoto() { const input = document.createElement('input'); input.type = 'file'; input.accept='image/*'; input.onchange = () => input.files[0] && uploadStory(input.files[0]); input.click(); }
async function uploadStory(file) { const fd = new FormData(); fd.append('file', file); const res = await api('/api/stories', {method:'POST',body:fd}); if (!res.ok) return alert(await res.text()); await loadStories(); }
function openStoryForUser(userId) { const list = stories.filter(s => s.user_id === userId); storyIndex = stories.findIndex(s => s.id === list[0]?.id); renderStory(); }
function renderStory() { const s = stories[storyIndex]; if (!s) return; const v = document.getElementById('stories-viewer'); v.hidden=false; document.getElementById('story-image').src=`/uploads/${encodeURIComponent(s.image_path)}`; document.getElementById('story-owner').textContent = s.username ? '@'+s.username : s.first_name; document.getElementById('story-caption').textContent=s.caption||''; document.getElementById('story-count').textContent=`${storyIndex+1}/${stories.length}`; api(`/api/stories/${s.id}/view`, {method:'POST'}); }
...
async function loadSettings() {
  const box = document.getElementById('settings'); if (!box) return; const res = await api('/api/settings'); if (!res.ok) return; const s = await res.json();
  box.innerHTML = `<div class="settings-card"><h2>Профиль</h2><label>Имя</label><input value="${esc(s.first_name)}" disabled><label>Username</label><input value="${s.username ? '@'+esc(s.username) : ''}" disabled><label>О себе</label><textarea id="settings-bio" maxlength="500">${esc(s.bio || '')}</textarea><button id="save-profile-btn" class="primary" type="button">Сохранить</button></div><div class="settings-card"><h2>Регистрация</h2><p>${s.phone_verified ? '✓ Номер телефона подтверждён' : 'Номер телефона ещё не подтверждён.'}</p><button id="settings-phone-btn" class="ghost" type="button">${s.phone_verified ? 'Поделиться номером ещё раз' : 'Поделиться номером'}</button></div><div class="settings-card"><h2>Уведомления</h2><p>${s.write_access ? '✓ Sqawe может присылать сообщения в Telegram.' : 'Разреши Sqawe присылать уведомления в Telegram.'}</p><button id="settings-push-btn" class="primary" type="button">${s.write_access ? 'Разрешение включено' : 'Включить Telegram-уведомления'}</button><label class="switch-row"><input id="push-enabled" type="checkbox" ${s.push_enabled ? 'checked' : ''}> Получать push-уведомления</label></div>`;
  document.getElementById('save-profile-btn').onclick = async () => { const fd = new FormData(); fd.append('bio', document.getElementById('settings-bio').value); const r = await api('/api/settings/profile', {method:'POST',body:fd}); if (r.ok) alert('Профиль сохранён.'); };
  document.getElementById('settings-phone-btn').onclick = requestPhone;
...
function requestPhone() {
  if (!tg?.requestContact) return alert('Telegram не поддерживает запрос номера на этой версии клиента.');
  tg.requestContact(async shared => { if (!shared) return; await api('/api/settings/phone-requested',{method:'POST'}); await loadSettings(); });
}

async function maybeShowRegistration() {
  const res = await api('/api/me'); if (!res.ok) return;
  const me = await res.json();
  if (!me.phone_verified && !me.phone_requested) document.getElementById('registration-modal')?.classList.remove('hidden');
}
document.getElementById('share-phone-btn')?.addEventListener('click', async () => { await requestPhone(); document.getElementById('registration-modal')?.classList.add('hidden'); });
document.getElementById('skip-phone-btn')?.addEventListener('click', () => document.getElementById('registration-modal')?.classList.add('hidden'));

(async () => { await loadFeed(); await loadStories(); await loadNews(); await loadPeople(); await updateNotificationBadge(); await maybeShowRegistration(); })();
