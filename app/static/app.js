...
function postImageMarkup(path, postId = null) {
  ...
  return `<img class="post-image" src="${src}" data-image-src="${src}" data-post-id="${postId || ''}" loading="lazy" alt="Фото" onclick="openImageViewerFromElement(this);">`;
}
...
async function loadFeed() {
  ...
  try{
    ...
    box.innerHTML=posts.map(post=>`<article class="post" id="post-${post.id}"><div class="post-head"><img class="avatar profile-click" src="${avatar(post.photo_url)}" onclick="openProfile(${post.user_id})" alt=""><div class="profile-click" onclick="openProfile(${post.user_id})"><div class="name">${esc(post.first_name)} ${esc(post.last_name||'')}${verifiedMark(post)}</div><div class="username">${post.username?'@'+esc(post.username):'Telegram user'}</div></div><div class="feed-friend-button">${getMiniFriendButton(post.user_id,post.friend_status||'none')}</div></div>${post.image_path?postImageMarkup(post.image_path,post.id):''}${post.caption?`<p class="caption">${esc(post.caption)}</p>`:''}${renderPostTags(post.tags)}<div class="meta"><button class="like ${post.liked?'liked':''}" type="button" onclick="likePost(${post.id},this)">${post.liked?'<span class="icon-heart filled">♥</span>':'<span class="icon-heart">♡</span>'} ${post.likes}</button><button class="comment-toggle" type="button" onclick="toggleComments(${post.id})"><span class="icon-comment"></span> ${post.comments||0}</button></div><div id="comments-${post.id}" class="comments" hidden></div></article>`).join('');
...
async function openImageViewerFromElement(el){if(!el)return;await openImageViewer(el.dataset.imageSrc||el.src,el.dataset.postId||null);}
async function openImageViewer(src,postId=null){const v=document.getElementById('image-viewer');if(!v)return;v.dataset.postId=postId?String(postId):'';v.dataset.authorId='';v.classList.add('active');v.setAttribute('aria-hidden','false');document.body.classList.add('image-viewer-open');const img=v.querySelector('.image-viewer-image');img.src=src;v.querySelector('.image-viewer-menu')?.classList.remove('active');const save=v.querySelector('#image-save-link');if(save){save.href=src;save.download=`sqawe-${postId||'photo'}.jpg`;}const del=v.querySelector('#image-delete-button');if(del)del.hidden=true;const author=v.querySelector('#image-viewer-author');if(!postId){if(author)author.hidden=true;return;}const pr=await api(`/api/posts/${postId}`);if(!pr.ok)return;const post=await pr.json();v.dataset.authorId=String(post.user_id);if(author)author.hidden=false;const av=v.querySelector('#image-viewer-author-avatar');if(av)av.src=avatar(post.photo_url);const nm=v.querySelector('#image-viewer-author-name');if(nm)nm.textContent=`${post.first_name||''} ${post.last_name||''}`.trim()||'Sqawe user';const un=v.querySelector('#image-viewer-author-username');if(un)un.textContent=post.username?`@${post.username}`:'';const tm=v.querySelector('#image-viewer-time');if(tm)tm.textContent=post.created_at||'';const lc=v.querySelector('#viewer-like-count');if(lc)lc.textContent=post.likes??0;const cc=v.querySelector('#viewer-comment-count');if(cc)cc.textContent=post.comments??0;const mr=await api('/api/me');if(mr.ok&&del)del.hidden=Number((await mr.json()).id)!==Number(post.user_id);await loadViewerComments(postId);}
async function loadViewerComments(postId){const box=document.querySelector('#image-viewer .image-viewer-comments');if(!box)return;const res=await api(`/api/posts/${postId}/comments`);if(!res.ok){box.innerHTML='<div class="comments-error">Не удалось загрузить комментарии.</div>';return;}const comments=await res.json();box.innerHTML=comments.length?comments.map(c=>`<button class="viewer-comment" type="button" onclick="openProfile(${c.user_id})"><img class="comment-avatar" src="${avatar(c.photo_url)}"><span><strong>${esc(c.first_name)} ${esc(c.last_name||'')}</strong><small>${esc(c.text)}</small></span></button>`).join(''):'<div class="comments-empty">Комментариев пока нет.</div>';}
async function deleteCurrentViewerPost(){const id=Number(document.getElementById('image-viewer')?.dataset.postId||0);if(!id||!confirm('Удалить эту публикацию?'))return;const res=await api(`/api/posts/${id}`,{method:'DELETE'});if(!res.ok)return alert(await res.text());closeImageViewer();await loadFeed();if(document.getElementById('profile-view')?.classList.contains('active'))await loadProfile();}
function closeImageViewer(){const v=document.getElementById('image-viewer');if(!v)return;v.classList.remove('active');v.setAttribute('aria-hidden','true');document.body.classList.remove('image-viewer-open');const img=v.querySelector('.image-viewer-image');if(img)img.src='';v.querySelector('.image-viewer-menu')?.classList.remove('active');}
function openImageViewerMenu(e){e?.stopPropagation();document.getElementById('image-viewer-menu')?.classList.toggle('active');}
document.getElementById('image-viewer-close')?.addEventListener('click',closeImageViewer);document.getElementById('image-viewer-dots')?.addEventListener('click',openImageViewerMenu);document.getElementById('image-delete-button')?.addEventListener('click',deleteCurrentViewerPost);document.getElementById('image-viewer-backdrop')?.addEventListener('click',closeImageViewer);document.getElementById('viewer-like-button')?.addEventListener('click',async()=>{const id=Number(document.getElementById('image-viewer')?.dataset.postId||0);if(!id)return;const res=await api(`/api/posts/${id}/like`,{method:'POST'});if(!res.ok)return;const d=await res.json();const icon=document.querySelector('#viewer-like-button .viewer-heart-icon');const count=document.getElementById('viewer-like-count');if(icon)icon.textContent=d.liked?'♥':'♡';if(count)count.textContent=d.likes;updateNotificationBadge();});document.getElementById('image-viewer-author-main')?.addEventListener('click',()=>{const id=Number(document.getElementById('image-viewer')?.dataset.authorId||0);if(id){closeImageViewer();openProfile(id);}});document.getElementById('viewer-comment-form')?.addEventListener('submit',async e=>{e.preventDefault();const id=Number(document.getElementById('image-viewer')?.dataset.postId||0);const input=document.getElementById('viewer-comment-input');const text=input?.value?.trim()||'';if(!id||!text)return;const fd=new FormData();fd.append('text',text);const r=await api(`/api/posts/${id}/comments`,{method:'POST',body:fd});if(!r.ok)return alert(await r.text());input.value='';await loadViewerComments(id);updateNotificationBadge();});document.addEventListener('pointerup',e=>{const image=e.target.closest('img[data-image-src]');if(!image)return;if(e.target.closest('button,a'))return;openImageViewerFromElement(image);});
