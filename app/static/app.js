...
async function loadBuildNumber() {
  ...
  try {
    ...
    if (!res.ok) { console.error('Build endpoint failed:', res.status); return; }
...
async function compressImageForUpload(file) {
  ...
  try {
    ...
    while (blob && blob.size > TARGET_BYTES && quality > 0.55) {
      quality -= 0.08;
...
async function uploadPostWithRetry(formData, attempts = 3) {
  let lastError = null;
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    try {
      ...
      if (![408, 425, 429, 500, 502, 503, 504].includes(res.status)) {
        return { ok: false, status: res.status, text: async () => text };
      }
...
postForm?.addEventListener('submit', async event => {
  ...
  const sourceFile = document.getElementById('image')?.files[0];
  const caption = document.getElementById('caption')?.value || '';
  const tagIds = document.getElementById('tag-ids')?.value || '';

  if (!sourceFile) {
    error.textContent = 'Для публикации нужно выбрать фото.';
    return;
  }
  ...
  try {
    const file = await compressImageForUpload(sourceFile);
    ...
    fd.append('tag_ids', tagIds);

    const res = await uploadPostWithRetry(fd, 3);

    if (!res.ok) {
      const message = await res.text();
      error.textContent = message || 'Не удалось опубликовать публикацию.';
      return;
    }
    ...
    error.textContent = 'Не удалось опубликовать. Проверь соединение и попробуй ещё раз.';
  } finally {
    if (button) {
...
document.addEventListener('click', e => { const image = e.target.closest('img[data-image-src]'); if (image) { e.preventDefault(); e.stopPropagation(); openImageViewer(image.dataset.imageSrc, image.dataset.postId || null, image.dataset.ownerId || null); } });
document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeImageViewer(); closeStoryViewer(); } });
function bindImageViewerAction(el, handler) {
  if (!el) return;
  const run = (event) => {
    event.preventDefault();
    event.stopPropagation();
...
bindImageViewerAction(document.getElementById('image-viewer-close'), closeImageViewer);
...
bindImageViewerAction(document.getElementById('image-viewer-delete'), deleteCurrentViewerPost);
