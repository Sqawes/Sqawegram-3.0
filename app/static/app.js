...
async function openImageViewer(src, postId = null, ownerId = null) {
  ...
  const del = viewer.querySelector('#image-delete-button');
  del.hidden = true;
  del.disabled = true;
  del.textContent = 'Удалить публикацию';

  if (postId) {
    try {
      const meRes = await api('/api/me');

      if (meRes.ok && ownerId !== null && ownerId !== '') {
        const me = await meRes.json();
        const canDelete = Number(ownerId) === Number(me.id);
...
document.addEventListener('click', e => { const image = e.target.closest('img[data-image-src]'); if (image) { e.preventDefault(); e.stopPropagation(); openImageViewer(image.dataset.imageSrc, image.dataset.postId || null, image.dataset.ownerId || null); } }, true);
document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeImageViewer(); closeStoryViewer(); } });
function toggleImageViewerMenu(event) { if (event) { event.preventDefault(); event.stopPropagation(); } const menu = document.getElementById('image-viewer-menu'); if (menu) menu.classList.toggle('active'); }
function handleViewerClose(event) { if (event) { event.preventDefault(); event.stopPropagation(); } closeImageViewer(); }
function handleViewerDelete(event) { if (event) { event.preventDefault(); event.stopPropagation(); } deleteCurrentViewerPost(); }
const viewerCloseButton = document.getElementById('image-viewer-close');
const viewerDotsButton = document.getElementById('image-viewer-dots');
const viewerDeleteButton = document.getElementById('image-viewer-delete');
const viewerDeleteButton2 = document.getElementById('image-delete-button');
viewerCloseButton?.addEventListener('click', handleViewerClose);
viewerDotsButton?.addEventListener('click', toggleImageViewerMenu);
viewerCloseButton?.addEventListener('pointerup', handleViewerClose);
viewerDotsButton?.addEventListener('pointerup', toggleImageViewerMenu);
viewerDeleteButton?.addEventListener('click', handleViewerDelete);
viewerDeleteButton2?.addEventListener('click', handleViewerDelete);
