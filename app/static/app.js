...
async function prepareUploadFile(file) {
  ...
  try {
    ...
    if (!blob) return file;
    return new File([blob], 'sqawe.jpg', { type: 'image/jpeg' });
  } catch (error) {
...
async function publishPostWithRetry(file, caption, tagIds, attempts = 3) {
  let lastError = null;

  for (let attempt = 1; attempt <= attempts; attempt++) {
    const fd = new FormData();
    fd.append('file', file, file.name || 'sqawe.jpg');
  ...
  return {
    ...
    message: lastError?.name === 'AbortError'
      ...
      : 'Не удалось опубликовать. Проверь соединение и попробуй ещё раз.'
  };
}
...
postForm?.addEventListener('submit', async event => {
  ...
  if (!originalFile) {
    if (error) error.textContent = 'Для публикации нужно выбрать фото.';
    return;
  }
  ...
  try {
    ...
    const result = await publishPostWithRetry(
      file,
      caption,
      tagIds,
      3
