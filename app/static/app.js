...
function postImageMarkup(path, postId = null, ownerId = null) {
  if (!path) return '';
  const src = `/uploads/${encodeURIComponent(path)}`;
  const safeSrc = String(src).replace(/'/g, "&#039;");
  return `<img class="post-image" src="${safeSrc}" data-image-src="${safeSrc}" data-post-id="${postId || ''}" data-owner-id="${ownerId || ''}" loading="lazy" alt="Фото" onclick="openImageViewer('${safeSrc}', ${Number(postId || 0)}, ${Number(ownerId || 0)})">`;
}
  ...
  try {
    ...
    box.innerHTML = posts.map(post => `
      <article class="post" id="post-${post.id}">
        <div class="post-head">
          <img class="avatar profile-click" src="${avatar(post.photo_url)}" onclick="openProfile(${post.user_id})" alt="">
          <div class="profile-click" onclick="openProfile(${post.user_id})">
            <div class="name">${esc(post.first_name)} ${esc(post.last_name || '')}${verifiedMark(post)}</div>
            <div class="username">${post.username ? '@' + esc(post.username) : 'Telegram user'}</div>
          </div>
          <div class="feed-friend-button">${getMiniFriendButton(post.user_id, post.friend_status || 'none')}</div>
        </div>
        ${post.image_path ? postImageMarkup(post.image_path, post.id, post.user_id) : ''}
        ${post.caption ? `<p class="caption">${esc(post.caption)}</p>` : ''}
        ${renderPostTags(post.tags)}
        <div class="meta">
          <button class="like ${post.liked ? 'liked' : ''}" onclick="likePost(${post.id}, this)"><span class="like-icon" aria-hidden="true">${post.liked ? '♥' : '♡'}</span> <span>${post.likes || 0}</span></button>
          <button class="comment-toggle" type="button" onclick="toggleComments(${post.id})"><span class="comment-icon" aria-hidden="true"></span> <span>${post.comments || 0}</span></button>
        </div>
    ...
    box.innerHTML = '<div class="empty">Не удалось загрузить ленту.<br>Попробуйте обновить.</div>';
  }
}
...
function renderProfilePost(post, ownProfile) {
  if (post.image_path) {
    return `<div class="profile-post">${postImageMarkup(post.image_path, post.id, post.user_id)}${post.caption ? `<p class="caption">${esc(post.caption)}</p>` : ''}${renderPostTags(post.tags)}</div>`;
  }
...
async function openProfileFriends(userId) {
  ...
  const backdrop = modal.querySelector('.profile-friends-backdrop');
  const close = modal.querySelector('.profile-friends-close');
  backdrop?.addEventListener('click', closeProfileFriends);
  close?.addEventListener('click', closeProfileFriends);
  requestAnimationFrame(() => modal.classList.add('active'));

  const list = modal.querySelector('#profile-friends-list');
  try {
    const res = await api(`/api/users/${Number(userId)}/friends`);
    if (!res.ok) {
      list.innerHTML = `<div class="empty">Не удалось загрузить друзей.<br>${esc(await res.text())}</div>`;
      return;
    }
    ...
    list.innerHTML = friends.map(friend => `
      <button type="button" class="profile-friend-item">
    ...
    `).join('');
    list.querySelectorAll('.profile-friend-item').forEach((button, index) => {
...
let imageViewerPostId = 0;
let imageViewerOwnerId = 0;
let imageViewerTags = [];
...
function toggleImageViewerMenu(event) {
  if (event) {
    event.preventDefault();
    event.stopPropagation();
  }
...
function renderViewerTags(tags) {
  ...
  if (!Array.isArray(tags) || !tags.length) {
    list.innerHTML = '<div class="viewer-tags-empty">Никого не отметили.</div>';
    return;
  }
  list.innerHTML = tags.map(u => {
    const name = [u.first_name, u.last_name].filter(Boolean).join(' ').trim() || u.username || 'Пользователь';
    return `
      <button class="viewer-tag-row" type="button" onclick="openProfileFromViewerTag(${Number(u.id)})">
        <img src="${avatar(u.photo_url)}" alt="">
        <span class="viewer-tag-info">
          <strong>${esc(name)}${verifiedMark(u)}</strong>
          <small>${u.username ? '@' + esc(u.username) : ''}</small>
        </span>
      </button>
    `;
  }).join('');
...
function openViewerTags() {
  const modal = document.getElementById('image-tags-modal');
  if (!modal) return;
  renderViewerTags(imageViewerTags);
  modal.hidden = false;
  requestAnimationFrame(() => modal.classList.add('active'));
}

function closeViewerTags() {
  const modal = document.getElementById('image-tags-modal');
  if (!modal) return;
  modal.classList.remove('active');
...
function openProfileFromViewerTag(userId) {
  closeViewerTags();
  closeImageViewer();
  openProfile(Number(userId));
}

function setViewerLikeState(liked, count) {
  const btn = document.getElementById('image-viewer-like');
  const icon = btn?.querySelector('.viewer-action-icon');
  const countEl = document.getElementById('image-viewer-like-count');
...
async function toggleViewerLike() {
  if (!imageViewerPostId) return;
  const btn = document.getElementById('image-viewer-like');
  if (btn) btn.disabled = true;
  try {
    const res = await api(`/api/posts/${imageViewerPostId}/like`, { method:'POST' });
    if (!res.ok) {
      const text = await res.text();
      alert(text || 'Не удалось поставить лайк.');
      return;
    }
    const data = await res.json();
    setViewerLikeState(data.liked, data.likes);
    updateNotificationBadge();
  } finally {
...
async function openImageViewer(src, postId = null, ownerId = null) {
  const viewer = document.getElementById('image-viewer');
  if (!viewer) return;

  imageViewerPostId = Number(postId || 0);
  imageViewerOwnerId = Number(ownerId || 0);
  imageViewerTags = [];
  ...
  const saveLink = document.getElementById('image-save-link');
  if (saveLink) {
    saveLink.href = src;
    saveLink.download = `sqawe-${imageViewerPostId || 'photo'}.jpg`;
  }
  ...
  const tagsBtn = document.getElementById('image-viewer-tags');
  if (tagsBtn) tagsBtn.hidden = true;

  const commentCount = document.getElementById('image-viewer-comment-count');
  if (commentCount) commentCount.textContent = '0';

  const commentsBox = document.querySelector('#image-viewer .image-viewer-comments');
  if (commentsBox) commentsBox.innerHTML = '<div class="comments-loading">Загрузка комментариев...</div>';

  const name = document.getElementById('image-viewer-name');
  const meta = document.getElementById('image-viewer-meta');
  const avatarEl = document.getElementById('image-viewer-avatar');
  if (name) name.textContent = 'Пользователь';
  if (meta) meta.textContent = '@username · загружается';
  if (avatarEl) avatarEl.src = '/static/default-avatar.svg';
  setViewerLikeState(false, 0);

  if (!imageViewerPostId) return;

  try {
    const [postRes, commentsRes, meRes] = await Promise.all([
      api(`/api/posts/${imageViewerPostId}`),
      api(`/api/posts/${imageViewerPostId}/comments`),
      api('/api/me')
    ]);

    if (postRes.ok) {
      const post = await postRes.json();
      imageViewerOwnerId = Number(post.user_id || imageViewerOwnerId);
      imageViewerTags = Array.isArray(post.tags) ? post.tags : [];

      if (name) name.innerHTML = `${esc([post.first_name, post.last_name].filter(Boolean).join(' ').trim() || 'Пользователь')}${verifiedMark(post)}`;
      if (meta) meta.textContent = `${post.username ? '@' + post.username : 'Telegram user'} · ${formatViewerTime(post.created_at)}`;
      if (avatarEl) avatarEl.src = avatar(post.photo_url);

      if (meRes.ok && del) {
        const me = await meRes.json();
        const canDelete = Number(me.id) === Number(post.user_id);
        del.hidden = !canDelete;
        del.disabled = !canDelete;
      }

      const likeCount = Number(post.likes || 0);
      setViewerLikeState(Boolean(post.liked), likeCount);
    }
    ...
    if (tagsBtn) {
      tagsBtn.hidden = imageViewerTags.length === 0;
      const tagsCount = document.getElementById('image-viewer-tags-count');
      if (tagsCount) tagsCount.textContent = imageViewerTags.length ? String(imageViewerTags.length) : '';
    }
  } catch (error) {
...
function renderViewerComments(comments) {
  const box = document.querySelector('#image-viewer .image-viewer-comments');
  if (!box) return;
  box.innerHTML = `<div class="comments-list">${comments.length ? comments.map(c => `
    <button class="viewer-comment" type="button" onclick="openProfileFromViewerTag(${Number(c.user_id)})">
      <img class="comment-avatar" src="${avatar(c.photo_url)}" alt="">
      <span class="comment-content">
        <strong>${esc(c.first_name || '')} ${esc(c.last_name || '')}</strong>
        <small>${c.username ? '@' + esc(c.username) : ''}</small>
        <span class="comment-text">${esc(c.text)}</span>
      </span>
    </button>
  `).join('') : '<div class="comments-empty">Комментариев пока нет.</div>'}</div>
  <form class="comment-form viewer-comment-form" onsubmit="submitViewerComment(event, ${imageViewerPostId})">
    <input id="viewer-comment-input-${imageViewerPostId}" maxlength="500" placeholder="Написать комментарий...">
...
async function submitViewerComment(event, postId) {
  ...
  if (commentsRes.ok) {
    const comments = await commentsRes.json();
    const count = document.getElementById('image-viewer-comment-count');
...
async function deleteCurrentViewerPost() {
  const id = Number(imageViewerPostId || 0);
  if (!id) return;
  if (!confirm('Удалить эту публикацию?')) return;
...
function closeImageViewer() {
  ...
  const img = v.querySelector('#image-viewer-image');
  if (img) img.src = '';
  imageViewerPostId = 0;
  imageViewerOwnerId = 0;
  imageViewerTags = [];
  const shell = v.querySelector('.image-viewer-shell');
  if (shell) {
...
function setupImageViewerGestures() {
  const viewer = document.getElementById('image-viewer');
  const shell = viewer?.querySelector('.image-viewer-shell');
  if (!viewer || !shell) return;

  shell.addEventListener('touchstart', e => {
    if (!viewer.classList.contains('active')) return;
    const t = e.changedTouches[0];
    imageViewerStartX = t.clientX;
    imageViewerStartY = t.clientY;
  ...
  shell.addEventListener('touchmove', e => {
    ...
    const t = e.changedTouches[0];
    const dx = t.clientX - imageViewerStartX;
    const dy = t.clientY - imageViewerStartY;
    if (Math.abs(dx) < Math.abs(dy) || dx < 0) return;
    shell.style.transform = `translate3d(${Math.min(dx, 220)}px,0,0)`;
    shell.style.opacity = String(Math.max(.58, 1 - dx / 520));
  }, { passive:true });
  ...
  shell.addEventListener('touchend', e => {
    ...
    const dx = t.clientX - imageViewerStartX;
    const dy = t.clientY - imageViewerStartY;
    shell.style.transition = 'transform .24s cubic-bezier(.22,.8,.2,1), opacity .24s ease';
    if (dx > 110 && Math.abs(dx) > Math.abs(dy)) {
      shell.style.transform = `translate3d(110%,0,0)`;
      shell.style.opacity = '0';
      setTimeout(closeImageViewer, 220);
    } else {
      shell.style.transform = '';
      shell.style.opacity = '';
    }
...
bindImageViewerAction(document.getElementById('image-viewer-dots'), toggleImageViewerMenu);
bindImageViewerAction(document.getElementById('image-delete-button'), deleteCurrentViewerPost);
bindImageViewerAction(document.getElementById('image-viewer-like'), toggleViewerLike);
bindImageViewerAction(document.getElementById('image-viewer-comments-btn'), () => {
  document.querySelector('#image-viewer .image-viewer-comments-section')?.scrollIntoView({ behavior:'smooth', block:'start' });
});
bindImageViewerAction(document.getElementById('image-viewer-tags'), openViewerTags);
bindImageViewerAction(document.getElementById('viewer-tags-close'), closeViewerTags);

document.getElementById('image-viewer-author')?.addEventListener('click', e => {
  e.preventDefault();
  e.stopPropagation();
  if (imageViewerOwnerId) {
    closeImageViewer();
    openProfile(imageViewerOwnerId);
  }
});

document.getElementById('image-save-link')?.addEventListener('click', () => closeViewerMenu());
document.getElementById('image-viewer-backdrop')?.addEventListener('click', closeImageViewer);
document.addEventListener('click', event => {
  const image = event.target.closest('.post-image');
  if (!image) return;
  if (event.target.closest('button,a')) return;
  const src = image.getAttribute('data-image-src') || image.getAttribute('src');
  const postId = Number(image.getAttribute('data-post-id') || 0);
  const ownerId = Number(image.getAttribute('data-owner-id') || 0);
  if (src) {
    event.preventDefault();
    event.stopPropagation();
    openImageViewer(src, postId, ownerId);
  }
});
document.querySelector('.viewer-tags-backdrop')?.addEventListener('click', closeViewerTags);
