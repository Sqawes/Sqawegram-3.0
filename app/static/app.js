...
const THEMES = {
  mono: { name: 'Classic Mono', vars: {} },
  graphite: { name: 'Graphite', vars: {} },
  ocean: { name: 'Ocean', vars: {} },
  lavender: { name: 'Lavender', vars: {} },
  sand: { name: 'Sand', vars: {} }
};
...
function renderThemeOptions() {
  ...
  box.innerHTML = Object.entries(THEMES).map(([key, meta]) => `
    <button type="button" class="theme-choice" data-theme="${key}" onclick="applyTheme('${key}')">
      <span class="theme-preview theme-preview-${key}"></span>
      <span><strong>${esc(meta.name)}</strong><small>${key === 'mono' ? 'Чёрный и белый' : key === 'graphite' ? 'Графит и холодный серый' : key === 'ocean' ? 'Глубокий синий и бирюза' : key === 'lavender' ? 'Тёмная лаванда и сирень' : 'Чёрный, кремовый и тёплый песок'}</small></span>
    </button>
  `).join('');
...
document.getElementById('open-theme-picker')?.addEventListener('click', openThemePicker);
...
async function openProfile(userId) {
  ...
  p.innerHTML = `<div class="profile-back"><button class="ghost" type="button" onclick="showView('feed-view')">← Назад</button></div><div class="profile-head"><img class="profile-avatar" src="${avatar(u.photo_url)}"><div class="profile-main"><div class="profile-name-row"><div><div class="profile-name">${esc(u.first_name)} ${esc(u.last_name || '')}</div><div class="profile-stats">${u.username ? '@' + esc(u.username) : 'Telegram user'}</div></div>${menu}</div><div class="profile-stats profile-friend-stat"><button type="button" class="profile-friend-count" onclick="openProfileFriends(${u.id})">${data.friend_count} друзей</button><span> · ${data.posts.length} постов</span></div>${onlineText(u.last_seen)}</div></div><div class="friend-button">${friendButtonForProfile(u.id, data.friend_status)}</div><div class="grid">${data.posts.length ? data.posts.map(post => renderProfilePost(post, own)).join('') : '<div class="profile-empty">Пока нет публикаций.</div>'}</div>`;
  ...
  list.innerHTML = friends.map(friend => `
    <button type="button" class="profile-friend-item" onclick="closeProfileFriends(); openProfile(${friend.id})">
      ...
      <span class="profile-friend-info">
        <span class="name">${esc(friend.first_name)} ${esc(friend.last_name || '')}</span>
...
(async () => { applyTheme(getStoredTheme()); loadBuildNumber(); await setFeedTab('people'); await loadPeople(); await updateNotificationBadge(); })();
