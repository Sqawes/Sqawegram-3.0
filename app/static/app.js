...
function postImageMarkup(path) {
  if (!path) return "";
  const src = `/uploads/${encodeURIComponent(path)}`;
  return `<img class="post-image" src="${src}" loading="lazy" onclick="openImageViewer('${src}')">`;
}
    ...
    for (const post of posts) {
      ...
      html.push(`
        <article class="post" id="post-${post.id}">
          ...
          ${postImageMarkup(post.image_path)}
...
async function openProfile(userId) {
  ...
  profile.innerHTML = `
    ...
    <div class="grid">
      ${data.posts.length ? data.posts.map(post => postImageMarkup(post.image_path)).join("") : '<div class="empty">Пока нет публикаций.</div>'}
    </div>
  `;
...
async function loadProfile() {
  ...
  box.innerHTML = `
    ...
    <div class="grid">
      ${data.posts.length ? data.posts.map(post => postImageMarkup(post.image_path)).join("") : '<div class="empty">У тебя пока нет публикаций.</div>'}
    </div>
  `;
}
...
function openImageViewer(src) {
  ...
  if (!viewer) {
    ...
    viewer.innerHTML = `<button class="image-viewer-close" type="button">×</button><img class="image-viewer-image" src="" alt="Фото">`;
    document.body.appendChild(viewer);
    viewer.addEventListener("click", event => {
      if (event.target === viewer || event.target.classList.contains("image-viewer-close")) closeImageViewer();
    });
  }
