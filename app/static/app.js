...
async function openImageViewerFromElement(el) {
  if (!el) return;
  await openImageViewer(el.dataset.imageSrc || el.currentSrc || el.src, el.dataset.postId || null);
}
...
async function openImageViewer(src, postId = null) {
  ...
  const save = document.getElementById('image-save-link');
  if (save) {
    save.href = src;
    save.download = `sqawe-${postId || 'photo'}.jpg`;
  }
  ...
  try {
    ...
    const un = document.getElementById('image-viewer-author-username');
...
function openViewerTags() {
  ...
  const tags = JSON.parse(document.getElementById('image-viewer')?.dataset.tags || '[]');
...
async function deleteCurrentViewerPost() {
  ...
  if (!id || !confirm('Удалить эту публикацию?')) return;
...
function startViewerSwipe(e) {
  if (e.target.closest && e.target.closest('.image-viewer-topbar, .image-viewer-menu, .viewer-action, .viewer-modal')) return;
  if (!document.getElementById('image-viewer')?.classList.contains('active')) return;
  viewerSwipeStartX = e.clientX ?? e.touches?.[0]?.clientX ?? null;
...
function endViewerSwipe(e) {
  ...
  if (endX === null) return;
  if (endX - viewerSwipeStartX > 90) closeImageViewer();
...
viewerDots?.addEventListener('pointerup', openImageViewerMenu, { passive: false });
viewerDots?.addEventListener('click', openImageViewerMenu);
viewerDelete?.addEventListener('pointerup', (e) => { e.preventDefault(); e.stopPropagation(); deleteCurrentViewerPost(); }, { passive: false });
viewerDelete?.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); deleteCurrentViewerPost(); });
viewerMenu?.addEventListener('pointerup', (e) => e.stopPropagation(), { passive: true });

const viewerSave = document.getElementById('image-save-link');
viewerSave?.addEventListener('pointerup', (e) => { e.stopPropagation(); }, { passive: true });
viewerSave?.addEventListener('click', (e) => { e.stopPropagation(); });
document.getElementById('image-viewer-backdrop')?.addEventListener('click', closeImageViewer);
...
viewerSwipeSurface?.addEventListener('pointerdown', startViewerSwipe, { passive: true });
viewerSwipeSurface?.addEventListener('pointerup', endViewerSwipe, { passive: true });
viewerSwipeSurface?.addEventListener('pointercancel', () => { viewerSwipeTracking = false; viewerSwipeStartX = null; }, { passive: true });
document.addEventListener('keydown', e => { if (e.key === 'Escape' && document.getElementById('image-viewer')?.classList.contains('active')) closeImageViewer(); });
