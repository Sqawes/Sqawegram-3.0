...
function postImageMarkup(path, postId = null) {
  ...
  return `<img class="post-image" src="${src}" data-image-src="${src}" data-post-id="${postId || ''}" loading="lazy" alt="Фото" role="button" tabindex="0" onclick="event.stopPropagation();openImageViewerFromElement(this);" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openImageViewerFromElement(this);}">`;
}
...
async function loadFeed() {
  ...
  try{
    ...
    if(meRes.ok) currentUserId=Number((await meRes.json()).id);
    ...
    box.innerHTML=posts.map(post=>`<article class="post" id="post-${post.id}"><div class="post-head"><img class="avatar profile-click" src="${avatar(post.photo_url)}" onclick="openProfile(${post.user_id})" alt=""><div class="profile-click" onclick="openProfile(${post.user_id})"><div class="name">${esc(post.first_name)} ${esc(post.last_name||'')}${verifiedMark(post)}</div><div class="username">${post.username?'@'+esc(post.username):'Telegram user'}</div></div><div class="feed-friend-button">${getMiniFriendButton(post.user_id,post.friend_status||'none')}</div></div>${post.image_path?postImageMarkup(post.image_path,post.id):''}${post.caption?`<p class="caption">${esc(post.caption)}</p>`:''}${renderPostTags(post.tags)}<div class="meta"><button class="like ${post.liked?'liked':''}" type="button" onclick="likePost(${post.id},this)" aria-label="Нравится"><span class="feed-heart-icon icon-outline-heart ${post.liked?'filled':''}" aria-hidden="true"></span><span>${post.likes||0}</span></button><button class="comment-toggle" type="button" onclick="toggleComments(${post.id})" aria-label="Комментарии"><span class="feed-comment-icon icon-outline-comment" aria-hidden="true"></span><span>${post.comments||0}</span></button></div><div id="comments-${post.id}" class="comments" hidden></div></article>`).join('');
  }catch(err){console.error(err);box.innerHTML='<div class="empty">Не удалось загрузить ленту. Попробуйте обновить.</div>';}
}
...
let viewerSwipeStartX = null;
let viewerSwipeTracking = false;
let viewerSwipePointerId = null;
let viewerSwipeDeltaX = 0;

async function openImageViewerFromElement(el) {
  if (!el) return;
  await openImageViewer(
    el.dataset.imageSrc || el.currentSrc || el.src,
    el.dataset.postId || null
  );
}
...
function maybeShowImageViewerSwipeHint() {
  const key = 'sqawe_photo_swipe_hint_seen_v1';
  if (localStorage.getItem(key) === '1') return;

  const hint = document.getElementById('image-viewer-swipe-hint');
  if (!hint) return;
  ...
  window.clearTimeout(window.__sqaweSwipeHintTimer);
  window.__sqaweSwipeHintTimer = window.setTimeout(() => {
    hideImageViewerSwipeHint();
  }, 10000);
}

function hideImageViewerSwipeHint() {
  const hint = document.getElementById('image-viewer-swipe-hint');
  if (!hint) return;
  hint.classList.remove('is-visible');
  window.clearTimeout(window.__sqaweSwipeHintTimer);
...
function getViewerImageUrl() {
  return document.getElementById('image-viewer')?.querySelector('.image-viewer-image')?.currentSrc ||
    document.getElementById('image-viewer')?.querySelector('.image-viewer-image')?.src || '';
}

async function saveViewerPhoto(e) {
  e?.preventDefault();
  e?.stopPropagation();

  const src = getViewerImageUrl();
  if (!src) return;
  ...
  try {
    ...
    a.href = objectUrl;
    a.download = `sqawe-photo-${document.getElementById('image-viewer')?.dataset.postId || 'photo'}.${(blob.type.split('/')[1] || 'jpg').replace('jpeg','jpg')}`;
    ...
    if (window.Telegram?.WebApp?.openLink) {
      window.Telegram.WebApp.openLink(src);
    } else {
...
async function openImageViewer(src, postId = null) {
  ...
  viewer.dataset.postId = postId ? String(postId) : '';
  viewer.dataset.authorId = '';
  viewer.dataset.tags = '[]';
  viewer.classList.add('active');
  viewer.setAttribute('aria-hidden', 'false');
  document.body.classList.add('image-viewer-open');

  const surface = document.getElementById('image-viewer-swipe-surface');
  if (surface) {
    surface.style.touchAction = 'none';
    surface.style.transform = '';
  }
  const body = viewer.querySelector('.image-viewer-body');
  if (body) {
  ...
  maybeShowImageViewerSwipeHint();

  const img = viewer.querySelector('.image-viewer-image');
  if (img) img.src = src;

  const menu = document.getElementById('image-viewer-menu');
  if (menu) menu.hidden = true;

  const del = document.getElementById('image-delete-button');
  if (del) del.hidden = true;

  const author = document.getElementById('image-viewer-author');
  if (!postId) {
    setHidden(author, true);
    return;
  }

  try {
    const pr = await api(`/api/posts/${postId}`);
    if (!pr.ok) return;

    const post = await pr.json();
    viewer.dataset.authorId = String(post.user_id);

    setHidden(author, false);

    const av = document.getElementById('image-viewer-author-avatar');
    if (av) av.src = avatar(post.photo_url);

    const nm = document.getElementById('image-viewer-author-name');
    if (nm) nm.textContent = post.username ? `@${post.username}` : `${post.first_name || ''} ${post.last_name || ''}`.trim();

    const lc = document.getElementById('viewer-like-count');
    if (lc) lc.textContent = post.likes ?? 0;

    const cc = document.getElementById('viewer-comment-count');
    if (cc) cc.textContent = post.comments ?? 0;

    const mr = await api('/api/me');
    if (mr.ok && del) {
      const me = await mr.json();
      del.hidden = Number(me.id) !== Number(post.user_id);
    }

    viewer.dataset.tags = JSON.stringify(post.tags || []);
  } catch (error) {
    console.error('image viewer post load failed', error);
  }
}
...
async function loadViewerComments(postId) {
  ...
  const res = await api(`/api/posts/${postId}/comments`);
  if (!res.ok) {
    list.innerHTML = '<div class="comments-error">Не удалось загрузить комментарии.</div>';
    return;
  }
  ...
  list.innerHTML = comments.length
    ? comments.map(c => `
        <button class="viewer-comment" type="button" onclick="closeViewerCommentModal();openProfile(${c.user_id})">
          <img class="comment-avatar" src="${avatar(c.photo_url)}" alt="">
          <span><strong>${esc(c.first_name)} ${esc(c.last_name || '')}</strong><small>${esc(c.text)}</small></span>
        </button>
      `).join('')
...
async function openViewerComments() {
  const postId = Number(document.getElementById('image-viewer')?.dataset.postId || 0);
  const modal = document.getElementById('viewer-comments-modal');
  if (!postId || !modal) return;
  modal.hidden = false;
  document.body.classList.add('viewer-modal-open');
  await loadViewerComments(postId);
}

function closeViewerCommentModal() {
  const modal = document.getElementById('viewer-comments-modal');
  if (modal) modal.hidden = true;
  document.body.classList.remove('viewer-modal-open');
}

function openViewerTags() {
  const modal = document.getElementById('viewer-tags-modal');
  const list = document.getElementById('viewer-tags-list');
  let tags = [];
  try { tags = JSON.parse(document.getElementById('image-viewer')?.dataset.tags || '[]'); } catch {}
  if (!modal || !list) return;

  list.innerHTML = tags.length
    ? tags.map(u => `
        <button class="viewer-person-row" type="button" onclick="closeViewerTagsModal();closeImageViewer();openProfile(${Number(u.id)})">
          <img class="avatar" src="${avatar(u.photo_url)}" alt="">
          <span><strong>${esc([u.first_name, u.last_name].filter(Boolean).join(' ') || u.username || 'Пользователь')}</strong><small>${u.username ? '@' + esc(u.username) : ''}</small></span>
        </button>
      `).join('')
    : '<div class="comments-empty">Отмеченных людей нет.</div>';

  modal.hidden = false;
  document.body.classList.add('viewer-modal-open');
}

function closeViewerTagsModal() {
  const modal = document.getElementById('viewer-tags-modal');
  if (modal) modal.hidden = true;
  document.body.classList.remove('viewer-modal-open');
}
...
async function deleteCurrentViewerPost() {
  const id = Number(document.getElementById('image-viewer')?.dataset.postId || 0);
  if (!id) return;
  if (!confirm('Удалить эту публикацию?')) return;
  ...
  if (menu) menu.hidden = true;
  await loadFeed();
  if (document.getElementById('profile-view')?.classList.contains('active')) await loadProfile();
}
...
function closeImageViewer() {
  ...
  hideImageViewerSwipeHint();
  closeViewerCommentModal();
  closeViewerTagsModal();
  ...
  if (body) {
    body.style.transition = 'transform .28s cubic-bezier(.22,.8,.24,1), opacity .2s ease';
...
function openImageViewerMenu(e) {
  e?.preventDefault();
  e?.stopPropagation();
  const menu = document.getElementById('image-viewer-menu');
  if (!menu) return;
...
function startViewerSwipe(e) {
  if (e.pointerType === 'mouse' && e.button !== 0) return;
  if (e.target.closest && e.target.closest('.image-viewer-topbar, .image-viewer-menu, .viewer-action, .viewer-modal, button, a, input, textarea')) return;
  if (!document.getElementById('image-viewer')?.classList.contains('active')) return;

  viewerSwipeStartX = e.clientX ?? null;
  viewerSwipePointerId = e.pointerId ?? null;
  viewerSwipeDeltaX = 0;
  viewerSwipeTracking = viewerSwipeStartX !== null;

  const surface = document.getElementById('image-viewer-swipe-surface');
  surface?.setPointerCapture?.(e.pointerId);
}

function moveViewerSwipe(e) {
  if (!viewerSwipeTracking || viewerSwipeStartX === null) return;

  const currentX = e.clientX ?? null;
  if (currentX === null) return;

  const delta = currentX - viewerSwipeStartX;
  viewerSwipeDeltaX = Math.max(0, delta);
  if (viewerSwipeDeltaX <= 0) return;
  ...
  const progress = Math.min(viewerSwipeDeltaX / width, 1);
  const scale = 1 - progress * 0.08;
  const opacity = 1 - progress * 0.35;

  body.style.transition = 'none';
  body.style.transform = `translate3d(${viewerSwipeDeltaX}px,0,0) scale(${scale})`;
  body.style.transformOrigin = 'center center';
  backdrop.style.transition = 'none';
  backdrop.style.opacity = String(opacity);
}

function endViewerSwipe(e) {
  if (!viewerSwipeTracking) return;

  const surface = document.getElementById('image-viewer-swipe-surface');
  surface?.releasePointerCapture?.(viewerSwipePointerId);

  const delta = viewerSwipeDeltaX;
  const viewer = document.getElementById('image-viewer');
  const body = viewer?.querySelector('.image-viewer-body');
  const backdrop = document.getElementById('image-viewer-backdrop');

  viewerSwipeTracking = false;
  viewerSwipeStartX = null;
  viewerSwipePointerId = null;
  viewerSwipeDeltaX = 0;

  if (!body || !backdrop) return;

  if (delta > Math.min(120, window.innerWidth * 0.22)) {
    body.style.transition = 'transform .24s cubic-bezier(.22,.8,.24,1), opacity .2s ease';
    body.style.transform = `translate3d(${window.innerWidth + 40}px,0,0) scale(.94)`;
    body.style.opacity = '.88';
    backdrop.style.transition = 'opacity .2s ease';
    backdrop.style.opacity = '0';
    window.setTimeout(closeImageViewer, 210);
  } else {
    body.style.transition = 'transform .36s cubic-bezier(.22,1.15,.32,1), opacity .24s ease';
...
const viewerDots = document.getElementById('image-viewer-dots');
const viewerDelete = document.getElementById('image-delete-button');
const viewerMenu = document.getElementById('image-viewer-menu');
const viewerSave = document.getElementById('image-save-link');

viewerDots?.addEventListener('pointerup', openImageViewerMenu);
viewerDots?.addEventListener('click', openImageViewerMenu);
viewerDelete?.addEventListener('pointerup', deleteCurrentViewerPost);
viewerDelete?.addEventListener('click', deleteCurrentViewerPost);
viewerSave?.addEventListener('pointerup', saveViewerPhoto);
viewerSave?.addEventListener('click', saveViewerPhoto);
viewerMenu?.addEventListener('pointerdown', e => e.stopPropagation());
viewerMenu?.addEventListener('pointerup', e => e.stopPropagation());
viewerMenu?.addEventListener('click', e => e.stopPropagation());

const viewerBackdrop = document.getElementById('image-viewer-backdrop');
viewerBackdrop?.addEventListener('click', closeImageViewer);
document.getElementById('viewer-comment-button')?.addEventListener('click', openViewerComments);
document.getElementById('viewer-tags-button')?.addEventListener('click', openViewerTags);
document.getElementById('viewer-comments-close')?.addEventListener('click', closeViewerCommentModal);
document.querySelector('#viewer-comments-modal .viewer-modal-backdrop')?.addEventListener('click', closeViewerCommentModal);
document.getElementById('viewer-tags-close')?.addEventListener('click', closeViewerTagsModal);
document.querySelector('#viewer-tags-modal .viewer-modal-backdrop')?.addEventListener('click', closeViewerTagsModal);

document.getElementById('image-viewer-author-main')?.addEventListener('click', () => {
  const id = Number(document.getElementById('image-viewer')?.dataset.authorId || 0);
  if (id) { closeImageViewer(); openProfile(id); }
});

document.getElementById('viewer-like-button')?.addEventListener('click', async () => {
  const id = Number(document.getElementById('image-viewer')?.dataset.postId || 0);
  if (!id) return;
  const res = await api(`/api/posts/${id}/like`, { method: 'POST' });
  if (!res.ok) return;
  const d = await res.json();
  const icon = document.querySelector('#viewer-like-button .viewer-heart-icon');
  const count = document.getElementById('viewer-like-count');
  if (icon) icon.classList.toggle('filled', !!d.liked);
  if (count) count.textContent = d.likes;
  updateNotificationBadge();
});

document.getElementById('viewer-comment-form')?.addEventListener('submit', async e => {
  e.preventDefault();
  const id = Number(document.getElementById('image-viewer')?.dataset.postId || 0);
  ...
  const fd = new FormData();
  fd.append('text', text);
  const r = await api(`/api/posts/${id}/comments`, { method: 'POST', body: fd });
  if (!r.ok) return alert(await r.text());
  input.value = '';
  await loadViewerComments(id);
  updateNotificationBadge();
});

const viewerSwipeSurface = document.getElementById('image-viewer-swipe-surface');
viewerSwipeSurface?.addEventListener('pointerdown', startViewerSwipe, { passive: false });
viewerSwipeSurface?.addEventListener('pointermove', moveViewerSwipe, { passive: false });
viewerSwipeSurface?.addEventListener('pointerup', endViewerSwipe, { passive: false });
viewerSwipeSurface?.addEventListener('pointercancel', endViewerSwipe, { passive: false });

document.addEventListener('keydown', e => {
  if (e.key === 'Escape' && document.getElementById('image-viewer')?.classList.contains('active')) closeImageViewer();
});
