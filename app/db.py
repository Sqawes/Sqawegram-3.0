...
def get_feed(current_user_id):
    ...
    rows = conn.execute("""
        SELECT
            ...
            CASE
                ...
                WHEN EXISTS(SELECT 1 FROM friendships f WHERE f.user_id=? AND f.friend_id=p.user_id) THEN 'friends'
                WHEN EXISTS(SELECT 1 FROM friend_requests fr WHERE fr.sender_id=? AND fr.receiver_id=p.user_id AND fr.status='pending') THEN 'pending'
                WHEN EXISTS(SELECT 1 FROM friend_requests fr WHERE fr.sender_id=p.user_id AND fr.receiver_id=? AND fr.status='pending') THEN 'incoming'
                ELSE 'none'
    ...
    if posts:
        ...
        tag_rows = conn.execute(f"""
            SELECT pt.post_id, u.id, u.username, u.first_name, u.last_name, u.photo_url, u.verified
            FROM post_tags pt
            JOIN users u ON u.id=pt.user_id
            WHERE pt.post_id IN ({placeholders})
            ORDER BY pt.post_id, pt.created_at
        """, ids).fetchall()
    else:
        tag_rows = []
    conn.close()
    tags_by_post = {}
    for row in tag_rows:
        tags_by_post.setdefault(row['post_id'], []).append(dict(row))
    for post in posts:
...
def get_post_stats(post_id, user_id):
    conn = get_conn()
    row = conn.execute(
        """
        SELECT
            (SELECT COUNT(*) FROM likes WHERE post_id=?) AS likes,
            EXISTS(SELECT 1 FROM likes WHERE post_id=? AND user_id=?) AS liked,
            (SELECT COUNT(*) FROM comments WHERE post_id=?) AS comments
        """,
        (post_id, post_id, user_id, post_id)
    ).fetchone()
    conn.close()
    return dict(row) if row else {"likes": 0, "liked": 0, "comments": 0}
