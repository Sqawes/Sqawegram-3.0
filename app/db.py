...
def upsert_user(user):
    ...
    conn.execute("""
        ...
        ON CONFLICT(id) DO UPDATE SET
            ...
            photo_url=CASE
                WHEN users.photo_url IS NOT NULL AND users.photo_url != ''
                     AND users.photo_url LIKE '/uploads/%'
                THEN users.photo_url
...
def update_profile(user_id, bio=None, photo_url=None):
    conn = get_conn()
    if bio is not None:
